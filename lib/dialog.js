...
import { htmlToBlocks, escapeMarkup } from "./markup.js";
...
export const NotificationDialog = GObject.registerClass(
  class NotificationDialog extends ModalDialog.ModalDialog {
    ...
    _init(n, actions) {
      ...
      // Author and relative time stay dimmed (inline alpha); the absolute
      // date/time keeps the full title color and is bold.
      const metaHead = [n.actor || n.reason, formatTimestamp(n.createdAt)]
        .filter(Boolean)
        .map(escapeMarkup)
        .join(" · ");
      const metaAbs = formatAbsolute(n.createdAt);
      const metaParts = [];
      if (metaHead) metaParts.push(`<span alpha="60%">${metaHead}</span>`);
      if (metaAbs) metaParts.push(`<b>${escapeMarkup(metaAbs)}</b>`);
      if (metaParts.length > 0) {
        const metaLabel = new St.Label({ style_class: "opn-dialog-meta" });
        metaLabel.clutter_text.set_markup(metaParts.join(" · "));
      ...
      if ((!detail.changes || detail.changes.length === 0) && blocks.length === 0 && !detail.comment) {
        content.add_child(new St.Label({ text: "нет деталей", style_class: "opn-dialog-meta" }));
      }
