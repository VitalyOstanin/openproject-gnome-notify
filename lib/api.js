...
import { parseNotifications, parseActivity } from "./parse.js";
...
export class OpenProjectClient {
  constructor(host) {
    ...
    // Activity (journal) resources are immutable, so cache them by href and only
    // fetch ones not seen before.
    this._activityCache = new Map();
  }

  // Scheme + authority of the configured host, used to resolve the root-relative
  // hrefs the API returns (e.g. "/openproject/api/v3/activities/123").
  _origin() {
  ...
  // Fetch an activity (journal) for its comment and field-change details. Returns
  // null on any non-OK response so a missing detail never breaks the poll. Cached
  // by href because activities never change once created.
  async getActivity(href) {
    if (!href) return null;
    if (this._activityCache.has(href)) return this._activityCache.get(href);
    await this._ensureToken();
    const origin = this._origin();
    if (!origin) throw new NotConfiguredError("host not set");
    // An absolute href from the server response is only followed if it stays on
    // the configured host; otherwise a compromised or MITM'd response could
    // redirect the request (with the auth token) to an attacker. The `+ "/"`
    // boundary prevents a prefix match like "https://host.evil.com".
    let url;
    if (href.startsWith("http")) {
      if (href !== origin && !href.startsWith(`${origin}/`)) {
        throw new Error("activity href origin mismatch");
      }
      url = href;
    } else {
      url = `${origin}${href}`;
    ...
    if (!msg) throw new NotConfiguredError(`invalid URL: ${url}`);
    const auth = this._authHeader();
    if (!auth) throw new TokenError("no token");
    const headers = msg.get_request_headers();
    ...
    const status = msg.get_status();
    if (status === Soup.Status.UNAUTHORIZED) throw new TokenError("unauthorized");
    if (status !== Soup.Status.OK) return null;

    const detail = parseActivity(
      JSON.parse(new TextDecoder().decode(bytes.get_data())),
    );
    this._activityCache.set(href, detail);
    return detail;
  }
