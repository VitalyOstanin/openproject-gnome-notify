...
import { formatTimestamp, formatAbsolute } from "./parse.js";
...
export function decodeEntities(text) {
  return String(text || "").replace(/&(#x?[0-9a-fA-F]+|[a-zA-Z]+);/g, (m, body) => {
    if (body[0] === "#") {
      ...
      // Guard String.fromCodePoint: an out-of-range or surrogate code point
      // would throw RangeError and abort the whole menu rebuild.
      const valid =
...
// Single-line Pango markup for a notification's author/time meta line, shared by
// the menu row and the full-content dialog. The author and relative time are
// dimmed via the given inline alpha; the absolute date/time keeps the full
// (title) color and is bold.
export function metaMarkup(n, dimAlpha) {
  const head = [n.actor || n.reason, formatTimestamp(n.createdAt)]
    .filter(Boolean)
    .map(escapeMarkup)
    .join(" · ");
  const abs = formatAbsolute(n.createdAt);
  const parts = [];
  if (head) parts.push(`<span alpha="${dimAlpha}">${head}</span>`);
  if (abs) parts.push(`<b>${escapeMarkup(abs)}</b>`);
  return parts.join(" · ");
}
