...
export const TasksIndicator = GObject.registerClass(
  class TasksIndicator extends PanelMenu.Button {
    ...
    _deficitHours(timelog) {
      // Yellow: today's shortfall; red: total plan minus logged over the range.
      const total = Math.max(0, timelog.planned - timelog.logged);
      const n = timelog.status === "yellow" ? timelog.todayDeficit : total;
    ...
    _taskItem(t) {
      ...
      item.activate = () => this._closeMenuThen(() => this._callbacks.onOpen(t));
