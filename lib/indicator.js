...
export const OpenProjectIndicator = GObject.registerClass(
  class OpenProjectIndicator extends PanelMenu.Button {
    _init(callbacks, iconsPath) {
      ...
      // notifications and tasks are visible without scrolling back up. Also cap
      // each column's height to the monitor so the columns scroll internally
      // instead of the whole menu overflowing into the PopupMenu's outer scroll.
      this.menu.connect("open-state-changed", (_menu, open) => {
        if (open) {
          this._capColumnHeights();
          this._scrollColumnsToTop();
        }
      });
    ...
    // Bound each list's height so it scrolls on its own; without a cap the
    // ScrollView expands to its content and the taller column pushes the whole
    // menu past the screen, handing scrolling to the PopupMenu's outer view.
    _capColumnHeights() {
      const mon = Main.layoutManager.primaryMonitor;
      if (!mon) return;
      // Fill the screen down to the work-area bottom: cap each list to the space
      // left after the panel and the menu chrome (titles, work-log summary,
      // footer, separators, ~150px). Staying just under the work area keeps the
      // PopupMenu's own outer scroll from engaging on top of the column scrolls.
      const wa = Main.layoutManager.getWorkAreaForMonitor(mon.index);
      const max = Math.max(200, wa.height - Main.panel.height - 150);
      const style = `max-height: ${max}px`;
      if (this._notifScroll) this._notifScroll.style = style;
      if (this._tasksScroll) this._tasksScroll.style = style;
    }
